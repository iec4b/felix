...
# "fido_demo" is the demonstration version of fido, set up for operation within
#  the sample_repository.

# this program requires the "pyperclip" module. 
# See https://pypi.python.org/pypi/pyperclip
...
# Here, the unique_id is set up to operate under the demo conditions.

unique_id  = "sample_id " + " ".join(arg for arg in args)

# Run the fido_demo.search function on the unique_id. See the file fido.py. 

metadata = fido_demo.search(unique_id)
...
# configuration opens only ogg files ending in ".ogg", but this can be adjusted
# as desired by changing the subprocess.call command below.  

elif "-a" in opts:
	print(f"Opening audio from {metadata['FolderPath']}.")
	subprocess.call(['open *.ogg'], cwd=metadata['FolderPath'], shell='TRUE')

# Open audio associated with the items using vlc CLI. The current
# configuration opens only ogg files ending in ".ogg", but this can be
...
# other files already queued. 
	...
	print(f"Opening audio from {metadata['FolderPath']}.")
	subprocess.call(['vlc --no-media-library *ogg'], cwd=metadata['FolderPath'], shell='TRUE')
